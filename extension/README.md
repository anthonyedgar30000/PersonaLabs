...
Scoring is deterministic and heuristic-based. Each card is first separated into deterministic signal layers, then weighted against the selected persona. PersonaLabs demonstrates explainable attentional modeling under varying metadata and signal quality conditions.
...
`SIGNAL_RICHNESS_SCORE` estimates how much reliable observable metadata exists for a card. It considers transcript availability, thumbnail-accessible text confidence, descriptive title quality, metadata completeness, duration stability, semantic consistency, and signal convergence across title/thumbnail/metadata/transcript sources.
...
Signal Richness: High (78/100)
Transcript availability: Available
Thumbnail OCR confidence: Medium
Metadata completeness: High
Signal convergence estimate: High
...
The model is intentionally transparent and limited. It does not diagnose, infer mental health state, or claim whether content is true. PersonaLabs models attentional friction and intentional alignment, not political correctness or objective truth. Low-confidence classifications should be read as directional observability estimates, not authoritative judgments.
...
Signal richness and confidence are separate from alignment. A video can be strongly aligned but low-confidence if metadata is sparse, and a misaligned video can be high-confidence when multiple observable sources converge. Weak metadata lowers confidence; strong signal convergence raises confidence.

For repeatable demos and tests, prefer high signal-richness videos over random browsing. Strong demo candidates include Linux/RHCSA tutorials, TED talks, documentaries, ambience channels, educational explainers, aquarium/bunny ambience, and long-form podcasts with captions. Low signal-richness examples such as meme spam, Shorts with minimal metadata, autogenerated uploads, ambiguous clickbait, and low-context thumbnails are useful for demonstrating uncertainty rather than final alignment quality.

See [`../demo_dataset.md`](../demo_dataset.md) for curated demo categories and expected outcomes.
