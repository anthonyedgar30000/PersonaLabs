(function attachHeadlineAnalyzer(root, factory) {
  ...
  function locatedTermsList(matches, location) {
    return matches.map((match) => `${match.term} (${location})`);
  }
  ...
  function buildReasons(label, scores, matchedTerms, source, governance) {
    ...
    if (label === "YELLOW") {
      if (governance.safeDomain.isSafeAnimalDomain) {
        reasons.push(`Marked yellow as uncertain because safe-domain animal content also contains unresolved title terms: ${locatedTermsList(matchedTerms.yellow, "title").join(", ") || "moderate risk"}.`);
      } else {
        reasons.push(`Marked yellow because mixed/context title terms were found: ${locatedTermsList(matchedTerms.yellow, "title").join(", ") || "moderate risk"}.`);
      }
    }
